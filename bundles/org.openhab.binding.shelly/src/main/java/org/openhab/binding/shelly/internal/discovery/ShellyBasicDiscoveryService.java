...
public class ShellyBasicDiscoveryService extends AbstractDiscoveryService {
    ...
    public static @Nullable DiscoveryResult createResult(String hostname, String ipAddress,
        ...
        ShellySettingsDevice devInfo = null;
        ...
        if ((devInfo != null) && (thingUID == null)) {
            ...
            final var name = hostname.startsWith(SERVICE_NAME_SHELLYPLUSRANGE_PREFIX) ? devInfo.hostname : hostname;
        ...
        if (thingUID != null) {
            final var name = properties.getOrDefault(PROPERTY_SERVICE_NAME, "Unknown").toString();

            final var type = substringBeforeLast(name, "-");
            ...
            logger.debug("{}: Adding Thing to Inbox (type {}, model {}, mode={})", name, type, model, mode);

            final var builder = DiscoveryResultBuilder.create(thingUID);
            builder.withProperties(properties).withLabel(name + "@" + ipAddress);
