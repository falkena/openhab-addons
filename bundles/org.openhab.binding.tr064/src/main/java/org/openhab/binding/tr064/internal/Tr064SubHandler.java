        ...
        if (checkProperties(scpdUtil)) {
            ...
            for (Channel channel : provided) {
                if (!channels.containsKey(channel.getUID())) {
                    ...
                    final String itemType = channel.getAcceptedItemType();
                    if ((description != null) && (deviceService != null) && (itemType != null)) {
                        final SCPDScpdType serviceRoot = scpdUtil.getService(deviceService.getServiceId()).orElse(null);
                        if (itemType.equals(description.getItem().getType()) && (serviceRoot != null)) {
                            final ActionType getAction = description.getGetAction();
                            try {
