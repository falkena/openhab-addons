...
public class IRobotCommonHandler extends BaseThingHandler {
    ...
    protected IRobotConnectionHandler connection = new IRobotConnectionHandler() {
        ...
        public void receive(final String topic, final String json) {
            ...
            if (JSONUtils.getAsJSONString("desired", tree) == null) {
                ...
                IRobotCommonHandler.this.receive(topic, tree);
            }
        }
    ...
    public void handleCommand(ChannelUID channelUID, Command command) {
        if (command instanceof RefreshType) {
            ...
            if (CHANNEL_CONTROL_FIND.equals(channelId)) {
            ...
            if (CHANNEL_CONTROL_CLEAN_PASSES.equals(channelId)) {
                ...
                request.addProperty("twoPass", command.equals(PASSES_2));
                connection.send(new Requests.DeltaRequest(request));
    ...
    private void reportMissionState(final JsonElement tree) {
        ...
        if (status != null) {
            ...
            State previousPhase = getCacheEntry(new ChannelUID(thingUID, MISSION_GROUP_ID, CHANNEL_MISSION_PHASE));
            // I7: cycle = "clean", 980: cycle = "quick"
            if (((previousPhase == null) || !previousPhase.equals(phase)) && !"none".equals(cycle)) {
                if ("run".equals(phase)) {
                    lastCleanMap.clear();
                    updateState(new ChannelUID(thingUID, MISSION_GROUP_ID, CHANNEL_MISSION_MAP), UnDefType.UNDEF);
                } else if ("hmPostMsn".equals(phase) || "hmUsrDock".equals(phase)) {
    ...
    private void reportPositionState(final JsonElement tree) {
        ...
        if (position != null) {
            ...
            if ((phase != null) && phase.equals("run") && (xPos != null) && (yPos != null)) {
                // I7: cycle = "clean", 980: cycle = "quick"
