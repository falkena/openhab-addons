...
Binding supports **drive** as additional thing.

```
Thing systeminfo:drive:<DriveId> [ index=<number> ]
```

The configuration of the thing gives the user the possibility to identify multiple hard drives available on the system.
...
```java
Bridge systeminfo:computer:work [interval_high=3, interval_medium=60]
```
